  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        ...
        <button className="flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-1.5 sm:py-2 bg-[#202020] text-white/40 rounded-lg transition-colors text-xs sm:text-sm">
          <EyeIcon className="w-3 h-3 sm:w-4 sm:h-4" />
          <span className="sm:inline">Show All</span>
        </button>
      </div>
      ...
      <div className="space-y-4">
        {upgradeData.map((upgrade, index) => (
          <motion.div
            ...
            className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 bg-[#202020] rounded-lg border border-gray-700/50 hover:bg-gray-800/70 transition-colors gap-3 sm:gap-4"
          >
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-white/5 rounded-lg flex items-center justify-center text-white font-semibold text-xs sm:text-sm">
                {upgrade.initials}
              </div>
